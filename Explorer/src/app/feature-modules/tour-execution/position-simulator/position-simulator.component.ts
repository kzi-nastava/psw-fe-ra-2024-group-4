...
import {
  Encounter,
  EncounterType,
} from '../../encounters/model/encounter.model';
...
export class PositionSimulatorComponent implements OnInit {
  ...
  loadActiveTour(): void {
    this.service.getActiveTour(this.user.id).subscribe((tour) => {
      if (tour && tour.id) {
        ...
        this.chatMessage =
          'Your tour is active! Track your location on the map, and as you approach key points, they will be marked as completed. To finish the tour, ensure you complete the last key point.';
      ...
      (position) => {
        if (
          ...
          if (this.activeEncounters.length > 0) {
            let activeUncompleted = this.activeEncounters.filter((encounter) =>
              encounter.instances?.some(
                (instance) =>
            ...
            for (let encounter of activeUncompleted) {
              ...
              if (distance <= 30) {
                // 30 meters
                // Show SweetAlert to notify user they are near the encounter
                Swal.fire({
                  title: `You are near the hidden location of the challenge "${encounter.title}"!`,
                  icon: 'info',
                  confirmButtonText: 'OK',
                }).then((result) => {
                  console.log(
                    `Encounter "${encounter.title}" is within 30 meters, starting timer.`
                  );
                  ...
                  setTimeout(() => {
                    this.service.getPositionByTourist(this.user.id).subscribe(
                      (updatedPosition) => {
                        if (
                          updatedPosition &&
                          updatedPosition.latitude !== undefined &&
                          updatedPosition.longitude !== undefined
                        ) {
                          ...
                          const updatedDistance = this.calculateDistance(
                            lat1,
                            lon1,
                            updatedLat2,
                            updatedLon2
                          ...
                          if (updatedDistance <= 30) {
                            ...
                            Swal.fire({
                              title: `Challenge "${encounter.title}" completed!`,
                              text: 'Congratulations, you have completed the challenge!',
                              icon: 'success',
                              confirmButtonText: 'OK',
                            });
                            this.encounterService.completeEncounter(
                              encounter.id!
                            );
                            console.log(
                              `Encounter "${encounter.title}" completed.`
                            );
                          } else {
                            console.log(
                              `User moved out of the 30-meter range after 30 seconds.`
                            );
                          }
                        } else {
                          console.error(
                            'Error fetching updated position after 30 seconds.'
                          );
                        }
                      },
                      (error) => {
                        console.error(
                          'Error fetching updated position:',
                          error
                        );
                      }
                    );
                  }, 20000); // 20 seconds for initial check
                });
              }
            }
          }
          ...
          console.error(
            'Current position is undefined. Setting to default values.'
          );
        }
      },
      (error) => {
      ...
      if (keyPoint && keyPoint.latitude && keyPoint.longitude) {
        ...
        if (distance < 50) {
          ...
          if (executionId !== undefined && keyPoint.id !== undefined) {
            ...
            console.error('TourExecution id or KeyPoint id is undefined:', {
              executionId,
    ...
    this.selectedTourPoints.forEach((keyPoint) => {
      if (keyPoint && keyPoint.latitude && keyPoint.longitude) {
        ...
        if (distance < 50 && keyPoint.id !== undefined) {
          console.log(
            `User is close to the challenge at key point: ${keyPoint.name}`
          );
        ...
        next: (data) => {
          ...
          this.encounters.forEach((encounter) => {
            ...
            if (userInstance?.status === 1) {
              console.log(
                `Encounter "${encounter.title}" is already completed by this user. Skipping.`
              );
              return;
            }
            if (userInstance?.status === 0) {
              console.log(
                `Encounter "${encounter.title}" is already active for this user. Skipping.`
              );
              return;
            }
            ...
            if (
              ...
              !this.processedEncounters.has(encounter.id)
            ) {
  ...
  private removeFarEncounters(): void {
    this.encounterService
      ...
      .subscribe({
        next: (data) => {
          const encountersInRadius = data.results.map(
          ...
          this.processedEncounters.forEach((encounterId) => {
            if (!encountersInRadius.includes(encounterId)) {
              ...
              console.log(
                `Encounter with ID ${encounterId} removed from processed list as it is out of radius.`
              );
            }
          });
        },
  ...
  handleMiscEncounter(challenge: Encounter | KeyPoint): void {
    ...
    if (this.isEncounter(challenge)) {
      actionDescription =
        challenge.miscData?.actionDescription ?? 'Unknown action';
      xp = challenge.xp ?? 0;
    } else {
      ...
      if (encounter) {
        actionDescription =
          encounter.miscData?.actionDescription ?? 'Unknown action';
        xp = encounter.xp ?? 0;
      } else {
  ...
  completeChallengeNoKeypoint(encounter: Encounter): void {
    ...
    this.service.getPersonInfo(this.user.id).subscribe({
      next: (person) => {
        ...
        console.log(
          `Fetched Person Data: XP=${currentXP}, Level=${currentLevel}`
        );
        ...
        this.encounterService.completeEncounter(encounter.id!).subscribe({
          next: () => {
            console.log(
              `Challenge completed for encounter: ${encounter.title}`
            );
  ...
  showEncounterDialog(keyPoint: KeyPoint): void {
    ...
    dialogRef.afterClosed().subscribe((encounterExists: boolean) => {
      if (!encounterExists) {
        console.warn(
          `Encounter for keyPoint "${keyPoint.name}" does not exist.`
        );
      } else {
  ...
  showEncounterDialogNoKeypoint(encounter: Encounter): void {
    ...
    if (this.processedEncounters.has(encounter.id!)) {
      console.log(
        `Encounter "${encounter.title}" has already been processed. Skipping.`
      );
      return;
    }
    ...
    dialogRef.afterClosed().subscribe((encounterActivated: boolean) => {
      if (!encounterActivated) {
        ...
        console.log(
          `Encounter "${encounter.title}" was successfully activated.`
        );
        this.loadActiveEncounters();
      }
    });
  ...
  completeKeyPoint(
    ...
    const isCompleted = this.tourExecution.completedKeys?.some(
      (key) => key.keyPointId === keyPointId
    );

    if (isCompleted) {
      console.log(
        `Key Point ${keyPointId} has already been completed. Skipping.`
      );
      return;
    }
    ...
    this.service.completeKeyPoint(executionId, keyPointId).subscribe({
      next: (result) => {
        console.log(
          `Key Point ${keyPointId} completed for execution ${executionId}`
        );
        ...
        if (!this.tourExecution.completedKeys) {
          ...
          const alreadyExists = this.tourExecution.completedKeys.some(
            (key) => key.keyPointId === completedKey.keyPointId
          );
          if (!alreadyExists) {
            this.tourExecution.completedKeys.push(completedKey);
            this.FinishTour(completedKey.keyPointId);
            console.log(
              'Updated Completed Keys:',
              this.tourExecution.completedKeys
            );
          } else {
            console.warn(
              'Key Point is already recorded as completed:',
              completedKey
            );
          }
        }
  ...
  abandonTourExecution(tourExecutionId?: number) {
    if (tourExecutionId !== null && tourExecutionId !== undefined) {
      this.tourExecutionService
        .abandonTourExecution(tourExecutionId)
        .subscribe({
  ...
  completeTourExecution(tourExecutionId?: number) {
    if (tourExecutionId !== null && tourExecutionId !== undefined) {
      this.tourExecutionService
        .completeTourExecution(tourExecutionId)
        .subscribe({
          next: (data: TourExecution) => {
            this.tourExecution = data;
          },
